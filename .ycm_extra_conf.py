...
header_file_extensions = ['.h', '.H', '.hxx', '.hpp', '.hh', '.tpp']
...
def GetAnyCompilationInfo(dirname, db):
    ...
    if source_files_in_dir:
        ...
        if compilation_info.compiler_flags_:
            print(
                "Getting flags for a header file, "
                "returning compilation flags for file: ", filename)
            return compilation_info
    return None


def TraverseSubdirsExcept(dir_to_traverse,
                          not_this_dir,
                          db,
                          db_root_dir='',
...
def GetCompilationInfoForFile(filename, dir_of_db, db):
    ...
    for extension in cpp_source_extensions:
        ...
        if os.path.exists(replacement_file):
            compilation_info = db.GetCompilationInfoForFile(replacement_file)
            if compilation_info.compiler_flags_:
                return compilation_info
...
def RemovePchFlag(flags):
    """
    precompiled headers don't work nicely with YCMD since the clang used in
    YCMD and used during compilation of the project can be completely
    different.
    Note: This assumes the project can compile without the PCH include. You
    could replace the PCH include with a -include directive if that's not
    true.
    """
    last_was_pch = False

    def helper_clang_pch(last_was_pch, flag):
        if last_was_pch:
            last_was_pch = False
            return ""
        elif flag == "-include-pch":
            last_was_pch = True
            return ""
        return flag

    flags = [helper_clang_pch(last_was_pch, flag) for flag in flags]

    # Remove PCH files if the compile commands came from GCC
    for i in range(len(flags)):
        if os.path.isfile(os.path.abspath(flags[i] + ".gch")):
            flags[i] = ""
            flags[i - 1] = ""
...
def FlagsForFile(filename, **kwargs):
    ...
    # added to a project.
    compilation_info = GetCompilationInfoForFile(filename, dir_and_db[0],
                                                 dir_and_db[1])
    ...
    return {
        'flags': RemovePchFlag(list(compilation_info.compiler_flags_)),
